...
if __name__ == "__main__":
    nx = np.arange( 500)
    ny = np.arange( 500)
    #latlonarray = np.array((np.zeros(500),np.zeros(500),[0,0]),dtype=float)
    latlonarray = np.zeros( 500 * 500 * 2).reshape(500,500,2)
    for x in nx:
        for y in ny:
            print("x = ", x," , y = ",y)
            lat, lon = calc_lat_lon( y, x, 33., 131.)
            latlonarray[y, x, 0] = lat
            latlonarray[y, x, 1] = lon
